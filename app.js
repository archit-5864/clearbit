...
app.post('/autocomplete', async (req, res) => {
  const input = req.body.input.trim();
  try {
    const suggestions = await getClearbitAutocomplete(input);
    res.json({ suggestions });
  } catch (error) {
    console.error('Clearbit Autocomplete Error:', error.message);
    res.status(500).json({ error: 'Error retrieving Clearbit autocomplete suggestions.' });
  }
});

async function getClearbitAutocomplete(query) {
  const clearbitAutocompleteUrl = `https://autocomplete.clearbit.com/v1/companies/suggest?query=${encodeURIComponent(query)}`;
  try {
    const response = await fetch(clearbitAutocompleteUrl);
    if (response.ok) {
      const data = await response.json();
      return data.map((company) => `${company.name} (${company.domain})`);
    } else {
      const data = await response.json();
      throw new Error(data.error.message);
    }
  } catch (error) {
    throw new Error('Error retrieving Clearbit autocomplete suggestions.');
  }
}
